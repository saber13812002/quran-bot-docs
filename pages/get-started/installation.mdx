...
<Tabs items={["مرحله ۱", "مرحله ۲","مرحله ۳"]}>
  <Tab>
    ### مرحله ۱: ایجاد پروژه
    با ایجاد یک پروژه جدید Next.js شروع کنید (اگر قبلاً آن را راه‌اندازی نکرده‌اید). متداول‌ترین روش استفاده از Create Next App است.

```bash copy filename=">ترمینال"
...
```
  ...
  <Tab>
  ### نصب Tailwind CSS 
  tailwindcss و وابستگی‌های آن را از طریق npm نصب کنید، و سپس دستور init را اجرا کنید تا فایل‌های `tailwind.config.js` و `postcss.config.js` ایجاد شوند.
...
![توضیح تصویر](./../../public/thumbnail.png)
